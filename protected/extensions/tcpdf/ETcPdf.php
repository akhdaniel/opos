...
/**
 * Include the the MYPDF class. IMPORTANT: Don't forget to customize its configuration
 * if needed.
 */
...
/**
 * ETcPdf is a simple wrapper for the MYPDF library.
...
{
   /**
    * The internal MYPDF object.
    *
    * @var object MYPDF
    */
   private $myMYPDF = null;
   ...
   public function __construct($orientation, $unit, $format, $unicode, $encoding)
   {
      ...
      if (!is_bool($unicode))
		...
		$this->myMYPDF = new MYPDF($orientation, $unit, $format, $unicode, $encoding);
    ...
    */
	...
	{
		if (is_object($this->myMYPDF) && get_class($this->myMYPDF)==='MYPDF') return call_user_func_array(array($this->myMYPDF, $method), $params);
		else throw new CException(Yii::t('ETcPdf', 'Can not call a method of a non existent object'));
	}

	public function __set($name, $value)
	{
	   if (is_object($this->myMYPDF) && get_class($this->myMYPDF)==='MYPDF') $this->myMYPDF->$name = $value;
	   else throw new CException(Yii::t('ETcPdf', 'Can not set a property of a non existent object'));
	}

	public function __get($name)
	{
	   if (is_object($this->myMYPDF) && get_class($this->myMYPDF)==='MYPDF') return $this->myMYPDF->$name;
