...
Summary per Product Report
Session <?php echo $session->name ?>

<?php $total=0 ?>

		
<?php echo sprintf("%-16s","Product")?><?php echo sprintf("%-10s","Qty") ?><?php echo sprintf("%10s","Total") ?>

----------------------------------------
<?php foreach($rows as $row):?>
<?php echo sprintf("%-20s",$row['name']) ?>

<?php echo sprintf("%20s",number_format($row['qty'],2)) ?>
<?php echo sprintf("%20s",number_format($row['s'],2)) ?>
