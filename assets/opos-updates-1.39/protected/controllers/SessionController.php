...
class SessionController extends Controller
{
	...
	private function getTotalSales($model){
		...
		foreach ($model->orders as $o){
			if($o->state == ORDER_NEW){
				Yii::app()->user->setFlash('error', "Cannot Close! Order #" . $o->number . " is still open.");
	...
	*/
	public function actionSummaryPerCategoryReport($id){
		Yii::app()->regkey->cekReg();
		...
		$rows = $this->summaryPerCategoryRows($id);

		$payments = $this->summaryPerPaymentRows($id);
	...
	/**
	 actionSummaryPerCategoryReport
	*/
	public function actionSummaryPerProductReport($id){
		Yii::app()->regkey->cekReg();
		...
		$rows = $this->summaryPerProductRows($id);
		$payments = $this->summaryPerPaymentRows($id);
	...
	/**
	 actionSummaryPerCategoryReport
	*/
	public function actionOrderPerPaymentReport($id){
		Yii::app()->regkey->cekReg();
		...
		$payments = $this->orderPerPaymentRows($id);
	...
	**************************************************************/
	...
	public function actionReportCategory(){
		...
		if(isset($_GET['Session']['start_date']) and isset($_GET['Session']['end_date'])){
			...
			$session 	= $this->summarySessions($start, $end);
			$rows 		= $this->summaryPerCategoryRows(null, $start, $end);
			$payments 	= $this->summaryPerPaymentRows(null, $start, $end);
			
			$submit = true;
			$print = isset($_GET['print']);

			if($print){
				$report = $this->renderPartial('reportPerCategory_txt',array(
					'session'=>$session,
					'payments'=>$payments,
					'rows'		=> $rows,
					'shop_name'=>$shop_name,
		...
		if(!$submit){
			$this->render('searchReport',array(
				'model'=>$model,
				'title'=>'Report Per Category'
			));
		}else{
			$this->render('reportPerCategory',array(
				'session'	=> $session,
				'payments'	=> $payments,
				'shop_name'	=> $shop_name,
				'rows'		=> $rows,
	...
	public function actionReportProduct(){
		...
		if(isset($_GET['Session']['start_date']) and isset($_GET['Session']['end_date'])){
			...
			//var_dump($start);die;
			$session 	= $this->summarySessions($start, $end);
			$rows 		= $this->summaryPerProductRows(null, $start, $end);
			$payments 	= $this->summaryPerPaymentRows(null, $start, $end);
			...
			if($print){
			$report = $this->renderPartial('reportPerProduct_txt',array(
				'session'=>$session,
				'payments'=>$payments,
				'rows'		=> $rows,
				'shop_name'=>$shop_name,
		...
		if(!$submit){
			...
			$this->render('reportPerProduct',array(
				'session'	=> $session,
				'payments'	=> $payments,
				'shop_name'	=> $shop_name,
				'rows'		=> $rows,
	...
	public function actionReportPayment(){
		...
		if(isset($_GET['Session']['start_date']) and isset($_GET['Session']['end_date'])){
			...
			$payments = $this->orderPerPaymentRows(null, $start, $end);
			
			$submit = true;
		
			
			if($print){
			$report = $this->renderPartial('reportOrderPerPayment_txt',array(
				'session'=>$session,
				'payments'=>$payments,
				'shop_name'=>$shop_name,
				'start'		=> $start,
		...
		if(!$submit){
			$this->render('searchReport',array(
				'model'=>$model,
				'title'=>'Report Order Per Payment'
			));
		}else{
			$this->render('reportOrderPerPayment',array(
				'session'	=> $session,
				'payments'	=> $payments,
				'shop_name'	=> $shop_name,
				'start'		=> $start,
