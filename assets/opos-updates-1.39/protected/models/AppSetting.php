...
class AppSetting extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('code, val', 'required'),
			array('code', 'length', 'max'=>50),
			array('val', 'length', 'max'=>200),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, code, val', 'safe', 'on'=>'search'),
		);
	}
	...
	public function findOrCreate($code, $default)
	{
		if (!$model = self::model()->find('code=:c', array('c'=>$code))) {
