...
<h1>Opening Control Session <?php echo $model->name; ?></h1>

<form action="<?php echo bu('/session/openCashbox')?>" method="POST" id="cashform">
...
<?php foreach($casboxLines as $i=>$cb):?>
	<tr>
		...
		<td><input type="text" class="cash-control" 
			...
			name="number_openings[<?php echo $cb['id'] ?>]" 
			value="<?php echo $cb['number_opening'] ?>" />
		</td>
...
$('#confirmClose').click(function(){
	if (confirm(\"Confirm Open Session?\")){
		\$('#cashform').submit();
	}
	return false;
});
